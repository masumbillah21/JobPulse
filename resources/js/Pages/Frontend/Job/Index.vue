<script setup lang="ts">
  ...
  const filteredJobs = computed(() => {
    ...
    return allActiveJobs.filter((job: {
      ...
      const cateId: never = job.job_category_id.toString() as never;
  ...
  <LayoutGuest>
    ...
    <main v-if="jobPageData">
      <BannerSection :bgImage="jobPageData?.featured_image">
        <SectionTitle :title="jobPageContent.banner.title" :description="jobPageContent.banner.description"
          ...
          <div class="flex flex-wrap">
            <div class="lg:pt-12 pt-6 w-full px-4">
              <div class="w-full lg:w-6/12 px-4 ml-auto mr-auto text-center text-slate-800 mb-5">
                <SectionTitle :title="jobPageContent.jobs.title" :description="jobPageContent.jobs.description"
                  :subtitle="jobPageContent.jobs.subtitle" />
              </div>
              <div class="flex">
                <div class="md:w-3/12 w-full md:mr-2 dark:text-black">
                  <h2 class="text-xl font-semibold px-4">Filters</h2>
                  <hr>
                  <div v-if="jobCategories" class="mt-4 bg-white dark:text-black rounded shadow p-4">
                    <h2 class="text-xl font-semibold">Categories</h2>
                    <hr>
                    <FormCheckRadioGroup class="mt-6" componentClass="w-full" v-model="form.categories"
                      name="categories" type="checkbox" :options="jobCategories" />
                  </div>
                  <div class="mt-4 bg-white rounded shadow p-4">
                    <h2 class="text-xl font-semibold">Level</h2>
                    <hr>
                    <FormCheckRadioGroup class="mt-6" componentClass="w-full" v-model="form.jobLevel" name="levels"
                      type="checkbox" :options="jobLevel" />
                  </div>
                  <div class="mt-4 bg-white rounded shadow p-4">
                    <h2 class="text-xl font-semibold">Job Type</h2>
                    <hr>
                    <FormCheckRadioGroup class="mt-6" componentClass="w-full" v-model="form.jobType" name="types"
                      type="checkbox" :options="jobTypes" />
                  </div>
                  <div class="mt-4 bg-white rounded shadow p-4">
                    <h2 class="text-xl font-semibold">Work Type</h2>
                    <hr>
                    <FormCheckRadioGroup class="mt-6" componentClass="w-full" v-model="form.workType" name="workType"
                      type="checkbox" :options="workType" />
                  </div>
                </div>
                <div
                  class="relative flex flex-col min-w-0 break-words bg-white text-slate-900 w-full mb-8 py-10 px-3 shadow-lg rounded-lg">
                  <p class="mb-2" v-if="form.categories.length > 0">
                    Category(s): <span v-for="category in form.categories"
                      class="font-bold bg-black rounded m-1 p-1 text-white">{{ jobCategories[category] }} </span>
                  </p>
                  <p class="mb-2" v-if="form.jobLevel.length > 0">
                    Level: <span v-for="level in form.jobLevel" class="font-bold bg-black rounded m-1 p-1 text-white">{{
      level }} </span>
                  </p>
                  <p class="mb-2" v-if="form.jobType.length > 0">
                    Job Type: <span v-for="job in form.jobType" class="font-bold bg-black rounded m-1 p-1 text-white">{{
      job }} </span>
                  </p>
                  <p class="mb-2" v-if="form.workType.length > 0">
                    Work Type: <span v-for="work in form.workType"
                      class="font-bold bg-black rounded m-1 p-1 text-white">{{ work }} </span>
                  </p>
                  ...
                  <p v-else class="text-center bg-red-500 text-white font-semibold rounded py-5 text-xl">No jobs found
                  </p>
                </div>
              </div>
            </div>
